...
total_production <- cap_aus %>% 
  ...
  reframe(total_prod = sum(value), species, year, type, value) %>% 
  ungroup() %>% 
  group_by(type, year) %>% 
  reframe(y_value = sum(value), total_prod) %>% 
  ungroup() %>% 
  unique()%>% 
...
prop_production <- total_production %>% 
  filter(type == "farmed") %>% 
  group_by(year) %>% 
  reframe(prop = (y_value/total_prod)*100, y_value, type) %>% 
  ungroup() %>% 
...
(sust_stocks <- ggplot(sust_cap, aes(x = year, y = prop*100, fill = status)) +
  geom_bar(stat = "identity") + 
    ...
    theme(text = element_text(size=12),
        ...
        legend.text = element_text(size=10),
        #legend.title = element_text("Sustainability status"),
        legend.position = "right",
        legend.key.width = unit(0.3, "cm"), 
        legend.key.height = unit(0.7, "cm"),
        legend.box.spacing = unit(-0.7, "cm"))+    #,
        #legend.margin = margin(c(0,0,0,0), "cm"))+
  labs(y="Number of stocks (%)", x="Year", fill = "Sustainability status")+
  guides(colour = FALSE) + 
...
aqu_prod_eFCR <- aqu_aus_3 %>%
 left_join(eFCRs, by = c("Taxa", "year" = "Year")) %>% 
  mutate(Taxa = case_when(Taxa %in% c("Salmons", "Trouts") ~ "Salmons and trouts", 
                          TRUE ~ Taxa))
...
#                             grepl("Barramundi", species) ~ "FW fishes",
#                              grepl("salmon|trout", species) ~ "Salmons and trouts",
#                              grepl("Freshwater crustaceans", isscaap_group) ~ "FW crustaceans",
#                              grepl("Marine fishes", species) ~ "Marine fishes ",
#                              grepl("Finfishes", species) ~ "Marine fishes ",
#                              grepl("marine crustaceans", species) ~ "Shrimps",
...
(eFCR_plot <- ggplot(prod_FCR %>% arrange(desc(eFCR))) + 
  geom_line( aes(x = year, y = eFCR, color= Taxa), linewidth = 1) + 
...
(t_levels_plot <- ggplot(all_df, aes(x = tl_group, y = farmed_tl_2015, color = tl_group)) + 
  ...
  scale_color_manual(values = blues) +
    ...
    labs(title = "Trophic levels of aquaculture \n species in Australia", y= "Mean effective trophic level (2015)", x = " Taxa group", color="Taxa group")+
    scale_y_continuous(labels = scales::comma))
...
(production + fed_unfed_aqua) / ( t_levels_plot + sust_stocks + eFCR_plot) + 
